...
var vot = (function(exports) {
	...
	*/
	...
	function getRuntimeLocaleVersion() {
		return resolveRuntimeLocaleVersion(String("1.11.3.6"), typeof GM_info !== "undefined" ? String(GM_info?.script?.version || "") : "");
	}
	...
	var SubtitlesWidget = class {
      ...
      >`;
		...
		updatePassedClasses(passedFlags) {
			...
			for (let i = 0; i < len; i += 1) tokenEls[i].classList.toggle("passed", passedFlags[i]);
			for (let i = len; i < tokenEls.length; i += 1) tokenEls[i].classList.remove("passed");
		}
		clearPassedClasses() {
			for (const tokenEl of this.renderedTokenEls) tokenEl.classList.remove("passed");
		}
      ...
      >
        ${this.renderTokens(tokens)}
			...
			this.renderedTokenEls = this.subtitlesBlock ? Array.from(this.subtitlesBlock.querySelectorAll("span[data-vot-token=\"1\"]")) : [];
		}
		update() {
  ...
  <path fill="currentColor" d="M7 15q1.25 0 2.125-.875T10 12t-.875-2.125T7 9t-2.125.875T4 12t.875 2.125T7 15m0 3q-2.5 0-4.25-1.75T1 12t1.75-4.25T7 6q2.025 0 3.538 1.15T12.65 10h8.375L23 11.975l-3.5 4L17 14l-2 2l-2-2h-.35q-.625 1.8-2.175 2.9T7 18"/>
		...
		*/
		...
		initUI(buttonPosition = "default") {
			...
			this.translationVolumeSlider = new Slider({
				...
				value: defaultVolume,
				max: this.data.audioBooster ? 900 : 100
			});
		...
		bindSettingsViewEvents() {
			...
			settingsView.addEventListener("update:account", async (account) => {
				...
				this.withInitializedOverlayView((overlayView) => {
					...
					const maxVolume = this.data.audioBooster ? 900 : 100;
	...
	*/
	...
	function syncTranslationLinkSnapshot(state, volumePercent) {
		...
		state.lastTranslationPercent = Math.max(0, Math.round(numeric));
	}
	/**
	...
	*/
	function applyVolumeLinkDelta({ state, fromType, newVolume, currentVideo, currentTranslation, translationMin, translationMax }) {
		...
		if (fromType === "video") {
			...
			const nextTranslation = clampInt(state.lastTranslationPercent + delta, translationMin, translationMax);
			state.lastTranslationPercent = nextTranslation;
			return { nextTranslation };
		}
		const normalizedTranslation = clampInt(Number.isFinite(newVolume) ? newVolume : currentTranslation, translationMin, translationMax);
	...
	*/
	...
	function getAutoVolumeMode(handler) {
		if (handler.data?.syncVolume || !handler.data?.enabledAutoVolume) return "off";
